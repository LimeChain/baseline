...
contract OrgRegistry is Ownable, ERC165Compatible, Registrar, IOrgRegistry {
    ...
    mapping(uint256 => Roles.Role) private roleMap;
    // address[] public parties;
    Org[] orgs;
    mapping(address => address) managerMap;

    event RegisterOrg(bytes32 _name, address _address, uint256 _role, bytes _messagingKey, bytes _zkpPublicKey);
    ...
    function registerOrg(
        ...
        emit RegisterOrg(_name, _address, _role, _messagingKey, _zkpPublicKey);
        return true;
    }
    ...
    function getOrg(address _address) external view returns (address, bytes32, uint256, bytes memory, bytes memory) {
        return (
    ...
    function getInterfaceAddresses()
        external
        view
        returns (bytes32[] memory, address[] memory, address[] memory, address[] memory)
    {
    ...
    function getOrgs()
        external
        view
        returns (address[] memory, bytes32[] memory, uint256[] memory, bytes[] memory, bytes[] memory)
    {
        address[] memory addresses = new address[](orgs.length);
        bytes32[] memory names = new bytes32[](orgs.length);
        uint256[] memory roles = new uint256[](orgs.length);
        ...
        return (addresses, names, roles, messagingKeys, zkpPublicKeys);
    }
}
